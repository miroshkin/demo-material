...
<body>
    <form id="form1" runat="server" style="font-family: Verdana; font-size: 12px;">
        ...
        <input type="button" class="button" style="top: 60px; position: absolute; width: 88px; text-align: center; right: 0px;"
            value="Найти" id="FindButton" onclick="$('#searchForTheFirstTime').val('false'); SearchProduct(encodeURIComponent($('#productName').val()), encodeURIComponent($('#productDrawing').val()));" 
             />
        <input type="button" class="button" value="Закрыть" id="NoButton" onclick="close_dialog('#SearchResultsWindow');"
        ...
        <asp:Label ID="NoProductsFoundMessage" CssClass="NoProductsFoundMessage" runat="server" Text="Изделия с указанными названием и чертежом/ГОСТ не найдены"></asp:Label>
    </form>
</body>
</html>
